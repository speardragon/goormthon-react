...
- 외부 접근: goormthon-5.goorm.training/\* 경로로 프론트엔드 애플리케이션 접근 가능
...
다음 파일에서 `goormthon-5`를 실제 팀 번호로 변경하세요

> ex. goormthon-5, goormthon-5 등
...
spec:
  template:
    spec:
      containers:
        - name: frontend
          image: 837126493345.dkr.ecr.ap-northeast-2.amazonaws.com/goormthon-5/frontend:latest #FIXME:
```
...
spec:
  rules:
    - host: goormthon-5.goorm.training #FIXME:
```

**kustomization.yaml**

```yaml
namespace: goormthon-5 #FIXME:
```
...
```bash
kubectl rollout restart deployment frontend-deployment -n goormthon-5
```
