...
- 외부 접근: goormthon-1.goorm.training/api/\* 경로로 백엔드 API 접근 가능
...
spec:
  template:
    spec:
      containers:
        - name: backend
          image: 837126493345.dkr.ecr.ap-northeast-2.amazonaws.com/goormthon-1/backend:latest #FIXME:
```
...
```bash
kubectl rollout restart deployment backend-deployment -n goormthon-1
```
